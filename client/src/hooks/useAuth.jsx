...
export function AuthProvider({ children }) {
  ...
  const doLogin = useCallback(
    async (payload) => {
      ...
      await refresh();
      reconnectSocket(); // ✅ fresh socket handshake picks up session cookie
      return res;
    },
    [refresh]
  );
  ...
  const doSignup = useCallback(
    async (payload) => {
      ...
      await refresh();
      reconnectSocket(); // ✅ same logic as login
      return res;
    },
    [refresh]
  );
  ...
  const doLogout = useCallback(async () => {
    ...
    await refresh();
    reconnectSocket(); // ✅ ensure socket drops any prior authed session
  }, [refresh]);
